...
from graphgps.custom_conv.gin_conv import GINEConv
...
class DelayGINEConvLayer(nn.Module):
    """Graph Isomorphism Network with Edge features (GINE) layer.
     PLUS DELAY! (not yet implemented)
    """
    def __init__(self, dim_in, dim_out, dropout, residual):
        ...
        # gin_nn has the stuff after the message passing - two linear layers
        gin_nn = nn.Sequential( 
            pyg_nn.Linear(dim_in, dim_out), nn.ReLU(),
            pyg_nn.Linear(dim_out, dim_out))
        
        self.model = GINEConv(gin_nn) # GINEConv is the message passing 
...
# not used when using custom_gnn
